...
class EvalReport:
    def __init__(self, rtype=ReportType.FASTA_REPORT):
        ...
        self.num_possibly_split_alignements = (
            0  # Multiple alignments that are candidate split alignments
        )
        ...
        self.num_real_alignments = (
            0  # A final number of actual alignments with a correct CIGAR string
        )
        ...
        self.num_missed_alignments = (
            0  # Alignements that missed genes, aligned outside genes
        )
        ...
        self.num_halfbases_hit = (
            0  # The number of reads whose alignment falling within an annotation
        )
        # covers at least half of the bases of the read

        self.num_lowmatchcnt = (
            0  # The number of reads with a very low match count from extended CIGAR
        )
        ...
        self.num_multi_exon_alignments = (
            0  # alignments spanning multiple exons / this can be correct
        )
        self.num_multi_gene_alignments = 0  # Alignments that overlap more then one gene / this is definitely incorrect

        self.num_inside_miss_alignments = (
            0  # Alignments that are inside genes, but missed all exons
        )
        ...
        self.num_multicover_exons = (
            0  # Exons covered by more than one part of a split alignment
        )
        ...
        self.num_possible_spliced_alignment = (
            0  # A number of alignments that are possibly spliced
        )
        # I.e. skipping an one or more exons between two covered exons
        self.num_hit_all = (
            0  # A number of alignemnts that hit a series of exons without skipping any
        )
    ...
    def toString(self):
        if self.rtype == ReportType.FASTA_REPORT:
            ...
            """ % (
                self.reflength,
                len(self.chromlengths),
                self.chromosomes(),
                self.num_alignments,
                self.num_unique_alignments,
                self.num_alignments - self.num_non_alignments,
                self.num_non_alignments,
                self.avg_mapping_quality,
                self.min_mapping_quality,
                self.max_mapping_quality,
                self.num_good_quality,
                self.num_zero_quality,
                self.num_match,
                self.num_mismatch,
                self.num_insert,
                self.num_delete,
                self.match_percentage,
                self.mismatch_percentage,
            ...
            """ % (
                self.totalGeneLength,
                self.num_genes,
                self.num_exons,
                self.num_multiexon_genes,
                self.max_exons_per_gene,
                self.min_gene_length,
                self.max_gene_length,
                self.avg_gene_length,
                self.min_exon_length,
                self.max_exon_length,
                self.avg_exon_length,
            )
            ...
            """ % (
                self.sum_bases_aligned,
                self.sum_read_length,
                self.percentage_bases_aligned,
                self.num_genes_covered,
                self.num_genes - self.num_genes_covered,
                self.num_genes,
                self.num_exons_covered,
                self.num_exons - self.num_exons_covered,
                self.num_exons,
                self.num_evaluated_alignments,
                self.num_hit_alignments,
                self.num_missed_alignments,
                self.num_exon_hit,
                self.num_exon_miss,
                self.num_halfbases_hit,
                self.num_lowmatchcnt,
                self.num_good_starts,
                self.num_good_ends,
                self.num_equal_exons,
                self.num_partial_exon_miss,
                self.num_almost_good,
                self.num_good_alignment,
                self.good_alignment_percent,
                self.num_bad_alignment,
                self.bad_alignment_percent,
                self.num_hit_all,
                self.hit_all_percent,
            )
            ...
            genename  number_of_exons  gene_hits / gene_covered_bases  [(exon_hits / exon_covered_bases)]...
                ...
                if len(self.expressed_genes) != len(self.gene_coverage):
                    raise Exception(
                        "ERROR: Gene expression and gene coverage dictionaries do not match in length! (%d <> %d)"
            ...
            if self.detect_new_annotations:
                if len(self.pot_new_annotations) > 0:
                    ...
                    """ % (
                        len(self.pot_new_annotations),
                        self.alignments_with_pna,
                    )
                else:
                    report += """\n
            ...
            genename: [transcript name (number of exons)]...
                ...
                for genename, alternate_splicing_info in list(
                    self.alternate_splicing.items()
                ):
