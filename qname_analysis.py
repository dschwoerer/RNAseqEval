...
def verbose_usage_and_exit():
    sys.stderr.write(
        "qname_analysis - Analyze qnames for multiple aligners and measures.\n"
    )
    sys.stderr.write("\n")
...
if __name__ == "__main__":
    ...
    if not os.path.isdir(os.path.join(os.getcwd(), resultsfolder)):
        sys.stderr.write(
            "\nCreating results folder and subfolders: %s"
            % os.path.join(os.getcwd(), resultsfolder)
        )
    ...
    for dataset in datasets:
        ...
        if not os.path.isdir(
            os.path.join(os.getcwd(), resultsfolder, dataset, "gmvsmm2")
        ):
            os.mkdir(os.path.join(os.getcwd(), resultsfolder, dataset, "gmvsmm2"))
        if not os.path.isdir(
            os.path.join(os.getcwd(), resultsfolder, dataset, "gmvsgmap")
        ):
    ...
    for dataset in datasets:
        ...
        for measure in measures:
            gm_qnames_filename = os.path.join(
                namesfolder, "gm", "gm_%s_test_%s.names" % (dataset, measure)
            )
            gmap_qnames_filename = os.path.join(
                namesfolder, "gmap", "gmap_%s_test_%s.names" % (dataset, measure)
            )
            mm2_qnames_filename = os.path.join(
                namesfolder, "mm2", "mm2_%s_test_%s.names" % (dataset, measure)
            )
            ...
            (
                ...
                gm_qnames_filename, gmap_qnames_filename, mm2_qnames_filename
            )
            statsAllList.append(
                (dataset, measure, gmcount, gmapcount, mm2count, countall)
            )
            ...
            filename = os.path.join(
                os.getcwd(), resultsfolder, dataset, "all", gmapname
            )
            with open(filename, "w") as f:
            ...
            (gmonly, mm2only, dictboth, gmcount, mm2count, countboth) = compare2files(
                gm_qnames_filename, mm2_qnames_filename
            )
            statsGMvsMM2List.append((dataset, measure, gmcount, mm2count, countboth))
            gmname = "gm_%s_%s_only.names" % (dataset, measure)
            filename = os.path.join(
                os.getcwd(), resultsfolder, dataset, "gmvsmm2", gmname
            )
            with open(filename, "w") as f:
                for line in gmonly:
                    f.write(line)
            ...
            mm2name = "mm2_%s_%s_only.names" % (dataset, measure)
            filename = os.path.join(
                os.getcwd(), resultsfolder, dataset, "gmvsmm2", mm2name
            )
            with open(filename, "w") as f:
                for line in mm2only:
                    f.write(line)
            ...
            filename = os.path.join(
                os.getcwd(), resultsfolder, dataset, "gmvsmm2", name
            )
            with open(filename, "w") as f:
            ...
            (gmonly, gmaponly, dictboth, gmcount, gmapcount, countboth) = compare2files(
                gm_qnames_filename, gmap_qnames_filename
            )
            statsGMvsGMAPList.append((dataset, measure, gmcount, gmapcount, countboth))
            gmname = "gm_%s_%s_only.names" % (dataset, measure)
            filename = os.path.join(
                os.getcwd(), resultsfolder, dataset, "gmvsgmap", gmname
            )
            with open(filename, "w") as f:
                for line in gmonly:
                    f.write(line)
            ...
            gmapname = "gmap_%s_%s_only.names" % (dataset, measure)
            filename = os.path.join(
                os.getcwd(), resultsfolder, dataset, "gmvsgmap", gmapname
            )
            with open(filename, "w") as f:
                for line in gmaponly:
                    f.write(line)
            ...
            filename = os.path.join(
                os.getcwd(), resultsfolder, dataset, "gmvsgmap", name
            )
            with open(filename, "w") as f:
                for line in dictboth.keys():
                    f.write(line)
