...
def test_cigars(samfile, fastaref):
    ...
    ] = utility_sam.HashSAMWithFilter(samfile, qnames_with_multiple_alignments)
...
if __name__ == "__main__":
    ...
    if mode == "test-pos":
        if len(sys.argv) != 4:
            sys.stderr.write(
                'Test CIGAR strings. Check if changing position results in a "better" CIGAR\n'
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "\t%s test-pos [SAM file] [FASTA reference]\n" % sys.argv[0]
            )
            sys.stderr.write("\n")
            exit(1)
