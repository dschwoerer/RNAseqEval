...
def analyze_chimeric_PAF(filename):
    ...
    for line in paffile:
        ...
        if qname == tname:
            ...
            if (
                abs(qstart - tstart) <= allowed_inacc
                and abs(qend - tend) <= allowed_inacc
            ):
                count += 1
...
def analyze_chimeric_SAM(filename):
    ...
    ] = utility_sam.HashSAMWithFilter(filename, qnames_with_multiple_alignments)
...
def verbose_usage_and_exit():
    sys.stderr.write(
        "get_chimer_from_paf - prints out the name of chimeric reads, from a paf file with overlaps.\n"
    )
    sys.stderr.write("\n")
...
if __name__ == "__main__":
    ...
    if mode == "analyze-PAF":
        if len(sys.argv) != 3:
            sys.stderr.write(
                "Analyzes a PAF file with overlaps to detect chimeric reads.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "%s %s <PAF file with overlaps>\n" % (sys.argv[0], sys.argv[1])
            )
            sys.stderr.write("\n")
            exit(1)
    ...
    if mode == "analyze-SAM":
        if len(sys.argv) != 3:
            sys.stderr.write(
                "Analyzes a SAM file with mappings to detect chimeric reads.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "%s %s <SAM file with mappings>\n" % (sys.argv[0], sys.argv[1])
            )
            sys.stderr.write("\n")
            exit(1)
        ...
        if len(sys.argv) != 4:
            sys.stderr.write(
                "Splits reads in a fasta file into two files according to a set of read names.\n"
            )
            sys.stderr.write(
                "One file contains reads from a given set of names, and the other file contains other reads.\n"
            )
            sys.stderr.write("Usage:\n")
            sys.stderr.write(
                "%s %s <FASTA file with reads> <file with read names>\n"
                % (sys.argv[0], sys.argv[1])
            )
            sys.stderr.write("\n")
            exit(1)
